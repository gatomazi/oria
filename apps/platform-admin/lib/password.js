...
// ── CÓPIA DECLARADA ──────────────────────────────────────────────────────────────────────────
// Origem: apps/panel/lib/auth/password.js (byte a byte, menos este cabeçalho e o 'use strict'
// original). NÃO é um require: o Railway builda só `/apps/platform-admin`, e `apps/panel` não
// existe na imagem deste service.
//
// Por que a cópia é segura aqui: o formato do hash (`scrypt$1$N$r$p$salt$hash`) é gravado no
// banco COMPARTILHADO e conferido por CHECK na tabela `platform_admins`. Se os parâmetros
// divergirem entre os dois apps, os hashes continuam verificáveis (N/r/p vêm do próprio hash). O
// teste `unit-password` compara este arquivo com o do painel e reprova se o formato mudar.
