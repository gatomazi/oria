...
test('pgStore cumpre o contrato do store (schema, perfis, produtos, lotes, fila, retry, histórico, isolamento)', opcoes, async () => {
  ...
  try {
    ...
    assert.equal(tabelasCreative[0].n, 12, 'as 12 tabelas creative_* precisam existir via migration (9 do core + creative_feedback da 0033 + creative_angles da 0034 + creative_enrichment_proposals da 0036)');
