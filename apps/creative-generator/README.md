...
- Fase C (cena com pessoas, sem flag nova): o request aceita `subjects` (até 4 pessoas, com faixa etária, relação com a principal e o produto que veste), `interaction` (catálogo em `creative_core/templates/interactions.json`) e `scene_picks` (reproduz os sorteios da cena) — todos exigem `plan_schema_version=2` (e `scene_picks`, `prompt_version=2`); em plano v1 o core recusa em vez de ignorar. `POST /v1/draft` e `POST /v1/feedback-snapshot` são puros (sem chave, sem provedor): montam, a partir de um plano persistido, o rascunho de "Copiar dados" (com "de novo" e "variação" prontos) e o snapshot de "Gostei / Não gostei". `GET /v1/contracts` traz `catalog.interactions` e `catalog.relations` para as telas.
